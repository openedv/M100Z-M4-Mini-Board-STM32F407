...
void oled_init(void)
{
    ...
    __HAL_RCC_GPIOA_CLK_ENABLE();                                                                           /* OLED_RW, OLED_RST */
    __HAL_RCC_GPIOB_CLK_ENABLE();                                                                           /* OLED_CS, OLED_DC, OLED_RD, OLED_D5/6/7 */
    __HAL_RCC_GPIOC_CLK_ENABLE();                                                                           /* OLED_D0/1/2/3/4 */
    
    gpio_init_struct.Pin = GPIO_PIN_4 | GPIO_PIN_7;                                                         /* OLED_RW, OLED_RST */
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;
    gpio_init_struct.Pull = GPIO_PULLUP;
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOA, &gpio_init_struct);
    
    gpio_init_struct.Pin = GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11;   /* OLED_CS, OLED_DC, OLED_RD, OLED_D5/6/7 */
    ...
    oled_wr_byte(0x50, OLED_CMD);                                                                           /* [3:0] : Frequency division factor; [7:4] : Frequency of oscillation */
    oled_wr_byte(0xA8, OLED_CMD);                                                                           /* Set the number of drivers */
    oled_wr_byte(0x3F, OLED_CMD);                                                                           /* Default 0x3F (1/64) */
    oled_wr_byte(0xD3, OLED_CMD);                                                                           /* Sets the display offset */
    oled_wr_byte(0x00, OLED_CMD);                                                                           /* The default is 0. */
    oled_wr_byte(0x40, OLED_CMD);                                                                           /* Set display start line, [5:0] : number of lines */
    oled_wr_byte(0x8D, OLED_CMD);                                                                           /* Charge pump setup */
    oled_wr_byte(0x14, OLED_CMD);                                                                           /* bit2: On/off */
    oled_wr_byte(0x20, OLED_CMD);                                                                           /* Sets the memory address mode */
    ...
    oled_wr_byte(0xC8, OLED_CMD);                                                                           /* Set COM scanning direction, bit3:0: normal mode; 1: Redefine the mode COM[n-1]->COM0, N: the number of driving ways */
    ...
    oled_wr_byte(0xEF, OLED_CMD);                                                                           /* 1-255, default 0x7F (brightness setting, larger is brighter) */
    oled_wr_byte(0xD9, OLED_CMD);                                                                           /* Set the pre-charge period */
    oled_wr_byte(0xF1, OLED_CMD);                                                                           /* [3:0]：PHASE 1;[7:4]：PHASE 2; */
    oled_wr_byte(0xDB, OLED_CMD);                                                                           /* [3:0] : PHASE 1; [7:4] : PHASE 2 sets the VCOMH voltage multiplier */
    oled_wr_byte(0x30, OLED_CMD);                                                                           /* [6:4] : 000:0.65 *vcc; 001:0.77 *vcc; 011:0.83*vcc */
    oled_wr_byte(0xA4, OLED_CMD);                                                                           /* Global display enabled, bit0:1: enabled; 0: Off (white screen/black screen) */
    oled_wr_byte(0xA6, OLED_CMD);                                                                           /* Set the display mode, bit0:1: reverse display; 0: Normal display */
    oled_wr_byte(0xAF, OLED_CMD);                                                                           /* Enable display */
